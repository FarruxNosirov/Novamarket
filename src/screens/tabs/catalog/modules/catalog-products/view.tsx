...
const CatalogProductsView = ({}): ReactElement => {
  ...
  let effect = async () => {
    try {
      let res = await requests.products.getProductsWithID(params.id);
  ...
  let brandsEffect = async () => {
    try {
      let res = await requests.products.getProductsWithBrand(params.id);
      setProducts(res.data.data);
    } catch (error) {
      console.log(error);
    }
  ...
  let shopEffect = async () => {
    try {
      let res = await requests.products.getProductWithShopID(params.id);
      setProducts(res.data.data);
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <View style={styles.container}>
      <BackHeader hasSearch={true} style={styles.header} name={params.name} />
      <View>
        <Image
        ...
        <SelectableFlter id={params.id} setProducts={setProducts} />
        <FlatList
